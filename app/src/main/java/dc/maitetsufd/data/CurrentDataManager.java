...
public class CurrentDataManager {
  ...
  private static final String MARU_COOKIE = "MARU_COOKIE";
  ...
  // 마루 쿠키 저장
  private static void saveMaruCookie(CurrentData currentData, SharedPreferences.Editor editor) {
    editor.putInt("maru_cookie_size", currentData.getMaruCookies().size());
    Set<String> keySet = currentData.getMaruCookies().keySet();
    int i = 0;
    for (String key : keySet) {
      editor.putString(MARU_COOKIE + i + KEY, key);
      editor.putString(MARU_COOKIE + i + VALUE, currentData.getMaruCookies().get(key));
      i++;
    }
  }
  ...
  public static CurrentData load(Context context) {
    ...
    loadMaruCookie(sharedPreferences, loadCurrentData);
  ...
  // 마루 로그인 정보 로드
  private static void loadMaruCookie(SharedPreferences sharedPreferences, CurrentData currentData) {
    int maruCookieSize = sharedPreferences.getInt("maru_cookie_size", 0);
    Map<String, String> maruCookie = new HashMap<>();
    for (int i = 0; i < maruCookieSize; i++) {
      String key = sharedPreferences.getString(MARU_COOKIE + i + KEY, "");
      String value = sharedPreferences.getString(MARU_COOKIE + i + VALUE, "");
      maruCookie.put(key, value);
    }
    currentData.setMaruCookies(maruCookie);
  }
